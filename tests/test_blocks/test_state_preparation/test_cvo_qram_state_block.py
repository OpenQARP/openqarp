...
def test_state_preparation():
    # CVO-QRAM layout: anc[0..n-2], anc_u[n-1], q_mem[n..2n-1]
    # After the circuit, all ancilla/q_u are |0>, state is in q_mem.
    # Index for a memory bitstring b: sum(b[j] << (n+j) for j in range(n))
    ...
    for dataset in set_datasets:
        block = CVOQRAMStateBlock(dataset)
        block.build()
        ...
        for bitstring, amplitude in dataset.items():
            idx = sum(b << (n + j) for j, b in enumerate(bitstring))
            prob = abs(sv[idx]) ** 2
...
def test_under_normalized_dataset_is_rejected():
    with pytest.raises(ValueError, match="equal to 1"):
        CVOQRAMStateBlock({(1, 0, 0): 0.5, (0, 1, 0): 0.5})


def test_over_normalized_dataset_is_rejected():
    with pytest.raises(ValueError, match="equal to 1"):
        CVOQRAMStateBlock({(1, 0, 0): 0.8, (0, 1, 0): 0.8})
